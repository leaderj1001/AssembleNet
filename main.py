...
def _train(epoch, train_loader, model, criterion, optimizer, args):
...
def _eval(epoch, val_loader, model, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
...
def main(args):
    ...
    g = Graph(args.dilation, args.m, args.hidden_size, args.max_level, args.per_n_nodes)
    model = Model(graph=g.graph, max_level=args.max_level, per_n_node=args.per_n_nodes)
